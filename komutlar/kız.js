...
const { tag, KızRole } = require('../macallan');
...
await member.setNickname(`${tag} ${isim} | ${yas}`)
  member.roles.add(macallan.KızRole); // kız rol ID'si
  ...
  .setColor("66FF00")                                                           
  .setTimestamp()
  .setDescription(`<@!${member.id}> <@&${macallan.KızRole}> • <@&${macallan.KızRole1}> • olarak kayıt edildi!`) 
